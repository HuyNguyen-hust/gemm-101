...
>
__global__ void gemm_v06_vectorized(size_t m, size_t n, size_t k,
                            const T alpha,
    ...
    for (size_t i{0U}; i < num_AB_thread_block_tiles; ++i)
    {
        ...
        for (size_t j{0U}; j < BLOCK_TILE_SIZE_K; ++j)
        {
            ...
            {   
                ...
                {
                    *reinterpret_cast<int4*>(&A_tmp[thread_tile_repeat_row_idx][thread_tile_row_idx*NUM_VECTOR_UNITS]) =
                        *reinterpret_cast<const int4*>(&A_thread_block_tile_shared_transposed[A_thread_block_tile_col_idx][A_thread_block_tile_row_idx + thread_tile_row_idx*NUM_VECTOR_UNITS]);
                }
            }
            ...
            size_t B_thread_block_tile_col_idx{warp_col_idx * WARP_TILE_SIZE_N +
                ...
                {
                    *reinterpret_cast<int4*>(&B_tmp[thread_tile_repeat_col_idx][thread_tile_col_idx*NUM_VECTOR_UNITS]) = 
                        *reinterpret_cast<const int4*>(&B_thread_block_tile_shared[B_thread_block_tile_row_idx][B_thread_block_tile_col_idx + thread_tile_col_idx*NUM_VECTOR_UNITS]);
                }
            }
...
template void launch_gemm_kernel_v06_vectorized<float>(size_t m, size_t n, size_t k,
                                              const float *alpha,
...
template void launch_gemm_kernel_v06_vectorized<__half>(size_t m, size_t n, size_t k,
                                              const __half *alpha,
...
template void launch_gemm_kernel_v06_vectorized<double>(size_t m, size_t n, size_t k,
                                              const double *alpha,
